...
7. ワークスペースをビルドする:
   ...
   CMake では、まず `pip install -U --ignore-installed -r constraints-pip.txt`（`pip` / `setuptools` / `wheel` の指定は **このファイルだけ**。`setuptools` は **80 未満**に抑え **colcon** と整合。`apt` 導入分の衝突を避けるため `--ignore-installed`）、続けて `pip install -c constraints-pip.txt -r requirements.txt`（Pydantic v2、nuscenes 系、Plotly 等）を行います。Python 3.12 では古い `setuptools` が `pkgutil.ImpImporter`（3.12 で削除）に依存しビルドが壊れるため、この手順にしています。`PIP_BREAK_SYSTEM_PACKAGES=1` と、キャッシュ先が書けない環境向けの `PIP_NO_CACHE_DIR=1` を CMake から付与します。事前導入する場合は `-DDRIVING_LOG_REPLAYER_V2_INSTALL_PYTHON_DEPS=OFF` を付け、同内容を手動で。`share/driving_log_replayer_v2/` に `requirements.txt`・`constraints-pip.txt` をインストールします。
