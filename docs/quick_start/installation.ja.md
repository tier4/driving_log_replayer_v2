...
7. ワークスペースをビルドする:
   ...
   CMake では、まず `pip install -U --ignore-installed -r constraints-pip.txt`（`pip` / `setuptools` / `wheel` の下限は **このファイルだけ**に記述。`apt` 導入分を pip がアンインストールしようとして失敗するのを避けるため `--ignore-installed` を付けます）、続けて `pip install -c constraints-pip.txt -r requirements.txt`（Pydantic v2、nuscenes 系、Plotly 等）を行います。Python 3.12 では古い `setuptools` が `pkgutil.ImpImporter`（3.12 で削除）に依存しビルドが壊れるため、この手順にしています。PEP 668 対策の `PIP_BREAK_SYSTEM_PACKAGES=1` を CMake から付与します。事前導入する場合は `-DDRIVING_LOG_REPLAYER_V2_INSTALL_PYTHON_DEPS=OFF` を付け、同内容を手動で。`share/driving_log_replayer_v2/` に `requirements.txt`・`constraints-pip.txt` をインストールします。
