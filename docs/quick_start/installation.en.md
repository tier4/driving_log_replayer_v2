...
7. Build the workspace:
   ...
   When `driving_log_replayer_v2` configures, it runs `pip install -U --ignore-installed -r constraints-pip.txt` (version floors for `pip` / `setuptools` / `wheel` are defined only in that file; `--ignore-installed` avoids errors when those packages were installed by `apt` without a pip `RECORD`), then `pip install -c constraints-pip.txt -r requirements.txt` (Pydantic v2, nuscenes stack, analyzer tools, etc.). This avoids **Python 3.12** issues where an old `setuptools` in pip’s build isolation hits `pkgutil.ImpImporter` (removed in 3.12). `PIP_BREAK_SYSTEM_PACKAGES=1` is set in `CMakeLists.txt` where needed. To skip, build with `-DDRIVING_LOG_REPLAYER_V2_INSTALL_PYTHON_DEPS=OFF` and install the same files yourself. They are also installed under `share/driving_log_replayer_v2/`.
