...
| 2 | scenario 自動生成 | `input_bag/` + map | `scenarios/auto_scenario.yaml` | なし | 1 |
...
| 6 | ケース集約解析 | `per_step/<tag>/per_step_delta.csv` 群 | `comparison/cases/` | 集約 | 1 |

Stage 3 (sim 実行) と Stage 5 (per-step) のケース定義はそれぞれ
`sim_runs.yaml` / `cases.yaml` で行う。scenario.yaml の
`Conditions.sim_runs_config` / `Conditions.cases_config` で参照し、
**両方未指定だとパイプラインは失敗**する。
