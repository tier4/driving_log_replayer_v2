...
出力: <base-dir>/comparison/param_sweep/
    ...
    param_sweep_derivation.md (§1 主軸: 傾き=パラメータ の数式的導出。レポートでは図の前),
    param_sweep_summary.md    (§2-§4 追認①実機ログ/追認②スイープ/整合。レポートでは図の後)
"""
...
def plot_sweep(
    ...
    fig = build_fig_sweep(
        ...
        title=f"{spec.label} ({spec.name}) 同定: 実機ログの傾き (根拠パネル) × "
        f"スイープ評価 (free-running rollout, N={identified['horizon']} の {metric_label} 最小化)",
    )
...
def write_summary(
    ...
    """車両モデルパラメータ同定の数式的根拠を Markdown 2 ファイルに分けて出力する。

    構成は **「傾き = パラメータ」の数式的導出 (§1) を主軸** に据え、その妥当性を
    独立な 2 つの角度で追認する:
      §2 追認① 実機ログでの傾き確認 (モデルを介さない直接推定)
      §3 追認② スイープ評価値での確認 (free-running rollout の N-step 誤差最小化)
    最後に §4 で導出軸・実機ログの傾き・スイープ評価値の三者が一致するかを判定する。

    レポート (step11) では **§1 (導出・テキスト) → 追認プロット群 → §2–§4 (追認・整合)**
    の順に並べたいので、図の前後で 2 ファイルに分割する:
      - `param_sweep_derivation.md` (= タイトル + 導入 + §1): プロットの **前** に置く。
      - `param_sweep_summary.md`    (= §2 + §3 + §4):         プロットの **後** に置く。
    """
    # --- 図の前: タイトル + 導入 + §1 主軸 (数式的導出。プロット無しのテキスト) ---
    deriv_lines = [
        "# パラメータ同定の数式的根拠\n",
        "車両モデルパラメータを **「傾き = パラメータ」の数式的導出 (§1) を主軸** に同定する。"
        "モデルの運動方程式を線形化し、実機ログの観測量を適切な縦軸・横軸に取ると、"
        "1 本の回帰直線の傾きがそのまま物理パラメータになる。"
        "その妥当性を独立な 2 つの角度で追認する: "
        "**追認① (§2) 実機ログでの傾き確認** (モデルを介さない直接推定) と、"
        "**追認② (§3) スイープ評価値** (free-running rollout の N-step 終端誤差最小化)。"
        "§4 で三者が一致するかを判定する。"
        "**追認のプロット (実機ログ根拠パネル・スイープ曲線) は本節の下に掲げる**。\n",
        "",
    ]
    deriv_lines += _build_derivation()  # §1 主軸: 数式的導出
    deriv_path = out_path.with_name("param_sweep_derivation.md")
    deriv_path.write_text("\n".join(deriv_lines) + "\n", encoding="utf-8")
    print(f"  Saved: {deriv_path}")

    # --- 図の後: §2 追認① → §3 追認② → §4 整合 (上掲プロットを読み取った値の表) ---
    conf_lines = []
    conf_lines += _build_log_confirmation(records)                      # §2 追認①
    conf_lines += _build_sweep_confirmation(records, horizons, stride)  # §3 追認②
    conf_lines += _build_discussion(records)                            # §4 整合
    out_path.write_text("\n".join(conf_lines) + "\n", encoding="utf-8")
    print(f"  Saved: {out_path}")


def _build_log_confirmation(records: list[dict]) -> list[str]:
    """追認①: §1 の導出式に従って実機ログから直接読んだ傾き (直接推定値) の一覧。

    各根拠パネル (_ev_*) が返す value は、§1 で導いた縦軸・横軸に実機ログを当てて
    1 次回帰した傾き (一部は切片・相互相関ピーク) であり、sweep を介さない一次情報。
    モデル積算 (§3) とは独立に、§1 の導出が実データでも成立することの直接確認になる。
    """
    lines = [
        "## 2. 追認①: 実機ログでの傾き確認\n",
        "§1 の各式が指定する縦軸・横軸に実機ログ "
        "(`/localization/kinematic_state` の速度・ヨーレート、`steering_status`、"
        "`control_cmd`、`localization/acceleration`) を当て、散布図に 1 次回帰を引いて "
        "傾きを読む (**上掲の根拠グラフ** = 各 sweep 図の根拠パネル / `acc_steady_evidence`)。"
        "**sweep を介さず実機ログだけで読める一次情報** であり、§1 の導出が実データでも "
        "成立することの直接確認になる。下表はそれら傾きの読み取り値。\n",
        "",
        "| param | 実機ログ直接推定 | 推定方法 |",
        "|---|---:|---|",
    ]
    for rec in records:
        ...
        for ev in rec.get("evidence") or []:
            ...
            if ev_v is None:
                continue
            lines.append(f"| {spec.name} | {ev_v:.4g} | {ev['desc']} |")
    lines += [
        "",
        "> 「実機ログ直接推定」は §1 の線形化式の回帰直線の傾き (= 物理パラメータ)。"
        "ただし `steer_bias` は $k_{us}$ 回帰の切片、`*_time_delay` は指令×実測の微分相互相関"
        "ピークであり、いずれも傾きではない別推定 (§1 補足参照)。",
        "",
    ]
    return lines


def _build_sweep_confirmation(
    records: list[dict],
    horizons: tuple[int, ...],
    stride: int,
) -> list[str]:
    """追認②: free-running rollout sweep で誤差を最小化する値 (別角度の確認)。

    §1/§2 の直接推定とは別経路 (モデルを N-step 積算した予測誤差から逆に最適値を探す)
    で同じ結論が出るかを示す。全パラメータの 1 次元 sweep を改善率の大きい順に並べる。
    """
    lines = [
        "## 3. 追認②: スイープ評価値での確認 (別角度)\n",
        f"独立な別角度として、free-running rollout (horizons={list(horizons)}, stride={stride}) の "
        "最大 horizon 終端誤差 RMSE を最小化するパラメータ値を求める (グリッド最小 + 近傍 3 点の "
        "放物線フィット)。モデルを N-step 積算した予測誤差から逆に最適値を探すので、§1/§2 の "
        "直接推定とは別経路。両者が一致すれば物理パラメータの補正として信頼できる。"
        "**上掲のスイープ曲線図** (k_us の RMSE vs パラメータ値) がこの最小化を可視化したもの。"
        "下表は全パラメータの結果を **改善率の大きい順** (= 直すと効く順) に並べる。\n",
        "",
        # メトリクス説明は §3 の従属内容なので H2→H3 に降格して節レベルを揃える
        # (metrics_description_md は他 step とも共有のため呼び出し側で降格する)。
        metrics_description_md().replace(
            "## メトリクス説明", "### メトリクス説明 (スイープの評価指標)", 1
        ),
        "",
...
def _build_derivation() -> list[str]:
    ...
    """
    return [
        "## 1. 主軸: 傾き = パラメータ の導出（一次情報）\n",
        "同定の主軸は、車両モデルの運動方程式を変形して "
        "**「傾きが物理パラメータそのものになる」ように縦軸・横軸を選ぶ** ことである。"
        "こうすると各パラメータは、実機ログ（§2）の散布図に 1 本の回帰直線を引くだけで読め、"
        "sweep（rollout 経由の同定 §3）とは独立した一次情報になる。"
        "以下、主要パラメータごとに導出を示す。\n",
        "",
