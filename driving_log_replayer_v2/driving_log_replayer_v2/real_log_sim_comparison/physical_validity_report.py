...
状態方程式へ直接代入する。評価は積分軌跡の位置誤差ではなく、<b>各時刻の瞬間残差</b>
\\(E = \\mathrm{{RHS}}(c) - \\mathrm{{LHS}}\\)（LHS は実測 \\(x,y\\) の SG 平滑化微分＝実測速度ベクトル）
で行う。積分（区間長・積分方法・入力ソースのノイズ特性など \\(c\\) と無関係な要因で
誤差の大小が変わってしまう）ではなく瞬間残差を使うのは、実車の入出力関係とモデル式の
入出力関係が一致しているかを直接見るため。
...
\\right)
\\]
を用い、全対象データセットの瞬間残差をプールして \\(c\\) を同定する。
</p>
<p>
...
<h3>x/y 式 瞬間方程式残差分布（上位 {n_dataset} データセット）</h3>
{resid_html}

<h3>代表データセットの軌跡比較（実車 vs x/y heading 補正式、見た目の一致度確認用）</h3>
<p>
初期状態を実車ログに合わせ、観測 \\(\\theta,v_x,\\dot\\theta\\) を固定入力として最適化後の x/y 右辺だけを
リセットなしで積分した参考軌跡（青破線）・\\(c=0\\) の軌跡（オレンジ点線）を
実車の軌跡（黒実線）と比較する。<b>\\(c\\) の定量評価は上の瞬間方程式残差分布・RMSE表を参照。</b>
この軌跡図は積分誤差が蓄積しない範囲で全体としての整合性を直感的に確認する用途であり、
区間が短く／実測 \\(\\theta,\\dot\\theta\\) を毎ステップ使うため baseline との差はほぼ視認できない
（積分誤差の蓄積を意図的に避けているため。詳細は瞬間残差の節を参照）。
...
""")
    ...
    perf_fig_traj = build_fig_xy_equation_traj(
        ...
        labels={
            ...
            "baseline_name": "baseline（heading補正なし、c=0）",
            "x_title": "Δx [m]",
            "y_title": "Δy [m]",
        },
        height=480,
