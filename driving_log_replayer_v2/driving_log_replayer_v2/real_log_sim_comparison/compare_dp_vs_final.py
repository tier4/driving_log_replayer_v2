...
def main():
    ...
    fig.suptitle(
        "DP出力 vs 最終planning/trajectory — 実機（行上段）/ 速度差（行下段）", fontsize=12
    )
    ...
    for i, d in enumerate(d_targets[:3]):
        ...
        ax.fill_between(t_vec, v_dp, v_final, alpha=0.2, color="blue", label=f"optimizer補正")
    ...
    ax_summary.plot(
        df_vel_real["tr"].values, df_vel_real["lon_vel"].values, "k-", lw=2, label="実機 actual速度"
    )
    ax_summary.plot(
        df_vel_sim["tr"].values,
        df_vel_sim["lon_vel"].values,
        "r-",
        lw=2,
        ls="--",
        label="シム actual速度",
