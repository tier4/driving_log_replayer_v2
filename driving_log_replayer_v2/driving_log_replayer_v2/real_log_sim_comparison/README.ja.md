...
| `sample/` | cloud / local 共通サンプル一式 (`scenario.yaml`, `cases.yaml`, `sim_runs.yaml`, `curve_config_miraikan.yaml`) + 手順 README (`sample/README.ja.md`)。ローカル実行出力は `sample/out/` (gitignore) |

> **build 後の install パス**
> CMakeLists が `sample/*` を `share/driving_log_replayer_v2/sample/real_log_sim_comparison/`
> に install する (登録済み cloud Web.Auto scenario が参照する install-share パスを維持)。

サンプルシナリオ: `sample/scenario.yaml`。`Datasets[0]` の UUID が
