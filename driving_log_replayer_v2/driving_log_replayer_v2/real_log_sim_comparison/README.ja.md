...
起動しない**。評価ノード `real_log_sim_comparison_evaluator` が 12 段階パイプラインを
subprocess で直接実行する。
...
| 11 | HTML レポート生成 (`step11_build_html_report`) | `comparison/` 配下の全 `*.fig.json` + 再生 HTML + 各 `.md` + 設定 YAML | `report.html` (バンドルフォルダ直下・plotly.js を 1 回インラインしクライアント描画する単一 HTML) | 1 |
...
| パス | 内容 |
|---|---|
| `step1_make_lite.py` … `step10_diagnose_curve.py` | 12 段階パイプラインの各 stage 実装（先頭 `stepN_` が実行順） |
...
1 ケースあたり `nstep_delta.csv`（全 horizon 統一スキーマ）+ 図 9 枚（うち `map_distribution.fig.json`）
...
| `overlay/error_timeseries_overlay.fig.json` | 全ケースを 1 枚に重ね描き（誤差時系列） |
...
| `<param>_sweep.fig.json` | 同定メトリクス・位置 RMSE vs param 曲線（同定値=赤線、仕様値=灰点線）+ 実機ログ根拠パネル（例: k_us は横加速度 vs ステア残差の understeer 勾配。sweep と独立な生データ直接観察によるクロスチェック） |
