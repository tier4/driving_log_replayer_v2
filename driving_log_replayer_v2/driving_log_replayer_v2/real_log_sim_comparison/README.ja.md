...
起動しない**。評価ノード `real_log_sim_comparison_evaluator` が 6 段階パイプラインを
subprocess で直接実行する。
...
| パス | 内容 |
|---|---|
| `step1_make_lite.py` … `step6_analyze_cases.py` | 6 段階パイプラインの各 stage 実装（先頭 `stepN_` が実行順） |
...
1 ケースあたり CSV (`per_step_delta.csv`) + 図 8 枚 + `summary.txt`。
