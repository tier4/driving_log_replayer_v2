...
def build_fig_equation_residual_hist(
    ...
    *,
    ...
    baseline_resid_samples=None,
    baseline_rmse_median: float = float("nan"),
) -> go.Figure:
    ...
    `baseline_resid_samples` を渡すと、同じ実測配列に baseline（補正なしデフォルト）
    パラメータを代入したときの残差分布を重ね描画し、tuned vs baseline の比較図にする。
    """
    ...
    bl_arr = None
    if baseline_resid_samples is not None:
        bl_arr = np.asarray(list(baseline_resid_samples), dtype=float)
        bl_arr = bl_arr[np.isfinite(bl_arr)]
        if bl_arr.size == 0:
            bl_arr = None

    mean = float(np.mean(arr))
    pooled_for_range = arr if bl_arr is None else np.concatenate([arr, bl_arr])
    lo, hi = (float(x) for x in np.percentile(pooled_for_range, [1, 99]))
    if not (hi > lo):  # 退化時のフォールバック
        lo, hi = float(pooled_for_range.min()), float(pooled_for_range.max() + 1e-9)
    ...
    fig.add_trace(go.Bar(
        ...
        marker_color="teal", opacity=0.75 if bl_arr is None else 0.55,
        name=f"tuned 残差 E[k]（n={arr.size}）",
    ))
    if bl_arr is not None:
        bl_counts, _ = np.histogram(bl_arr, bins=80, range=(lo, hi))
        fig.add_trace(go.Bar(
            x=centers.tolist(), y=bl_counts.tolist(), width=float(edges[1] - edges[0]),
            marker_color="orange", opacity=0.55, name=f"baseline 残差 E[k]（n={bl_arr.size}）",
        ))
        fig.update_layout(barmode="overlay")
    fig.add_vline(x=0.0, line=dict(color="black", width=1.5))
    fig.add_vline(
        x=mean, line=dict(color="crimson", width=1.5, dash="dot"),
        annotation_text=f"tuned 平均 {mean:.3g}", annotation_position="top right",
    )
    fig.update_xaxes(title_text=f"方程式残差 E[k] = RHS − LHS  [{unit_label}]", range=[lo, hi])
    fig.update_yaxes(title_text="サンプル数")
    extra = f"、tuned 残差 RMSE 中央値 {rmse_median:.3g} {unit_label}" if np.isfinite(rmse_median) else ""
    if bl_arr is not None and np.isfinite(baseline_rmse_median):
        extra += f"、baseline 残差 RMSE 中央値 {baseline_rmse_median:.3g} {unit_label}"
    return apply_base_layout(
        fig,
