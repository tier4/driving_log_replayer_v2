...
# 評価する horizon 群。短期(N=10〜40)・中長期(N=100, N=200) を等重みで集約する。
# N=200 は 30fps で 6.67 秒先。C ライブラリは任意 horizons に対応 (max(H) まで一括積分)。
HORIZONS = (10, 20, 30, 40, 100, 200)
...
# N=100, N=200 は 100 DS サンプルの reference (無補正) 誤差 p50 を基準に設定。
# N=100: yaw_p50≈1.09, lat_p50≈15.4, long_p50≈49.9 → 切りよく丸め
# N=200: yaw_p50≈2.12, lat_p50≈66.6, long_p50≈154.2 → 切りよく丸め
# N≥100 では open-loop 積分ドリフトで LAT/LONG が爆発的に増大するため、
# baseline と同程度でクリップする (各 DS の nlat/nlong ≈ 1 を起点に改善を測る設計)。
YAW_FLOOR  = {10: 0.06, 20: 0.12, 30: 0.18, 40: 0.24, 100: 1.0,  200: 2.0}    # deg
LONG_FLOOR = {10: 1.0,  20: 2.0,  30: 3.25, 40: 4.5,  100: 50.0, 200: 150.0}  # cm
LAT_FLOOR  = {10: 0.3,  20: 0.6,  30: 0.9,  40: 1.2,  100: 15.0, 200: 65.0}   # cm
