...
def delay_command(cmd: np.ndarray, delay_s: float, dt: float) -> np.ndarray:
    """Return the command shifted by the dead time (edge-held, grid-rounded)."""
    return _delay(np.asarray(cmd, dtype=float), max(0, int(round(delay_s / dt))))
