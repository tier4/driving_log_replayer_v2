...
2. **インストール済 param yaml が `$(env DIFFUSION_PLANNER_ONNX_PATH ...)` を含むこと**
   （`_assert_installed_param_honors_env`）。未反映なら `RuntimeError` を送出し、何が必要かを示す。
...
ローカルのソース編集だけではクラウドに効かない。以下はユーザーが行う:

1. **変更 A のクラウド反映**: launcher サブリポジトリ（autoware_launch）に A をコミットし、親リポの
   `autoware.repos` の launcher `version:` を更新する（`vcs import` で同期）。
