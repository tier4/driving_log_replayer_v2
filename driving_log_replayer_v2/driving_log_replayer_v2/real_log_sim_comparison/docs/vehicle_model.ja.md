...
$$\dot a = -\frac{a - a_{target}}{\tau_{a}}, \qquad a_{target} = a_{cmd} + \mathrm{poly}(v), \qquad \dot v = a + a_{slope}$$
...
$$\dot a = -\frac{a - a_{target}}{\tau}, \qquad a_{target} = a_{cmd}(t - T) + \mathrm{poly}(v)$$
...
**残る差 is 無駄時間 T のみ**: C++ は入力キューが単一で τ だけ throttle/brake 分離（T は共通）、ビューアは
\(T_{thr}/T_{brk}\) も分離する。
