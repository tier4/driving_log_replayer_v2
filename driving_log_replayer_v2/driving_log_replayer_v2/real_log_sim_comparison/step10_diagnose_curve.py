...
def plot_detailed(real: dict, sim: dict, cfg: RuntimeConfig, sim_name: str = "シム") -> None:
    ...
    fig = build_fig_curve_divergence(
        {
            "t": t_vec, "rv": rv, "sv": sv, "rc": rc, "sc": sc,
            "rs": rs, "ss": ss, "yaw_diff": yaw_diff,
            "dists": dists, "lons": lons, "lats": lats,
        },
        scenario_name=cfg.scenario_name, sim_name=sim_name,
    )
    out_dir = cfg.out_dir / "curve_diag"
    out_dir.mkdir(parents=True, exist_ok=True)
    write_fig_json(fig, out_dir / "curve_divergence")
