...
def main():
    ...
    if not df_obj_real.empty:
        print(
            f"  実機: 平均{df_obj_real['n_objects'].mean():.1f}物体, "
    ...
    if not df_obj_sim.empty:
        print(
            f"  シム: 平均{df_obj_sim['n_objects'].mean():.1f}物体, "
    ...
    if not df_obj_real.empty:
        ax_obj.plot(
            df_obj_real["t_rel"].values,
            df_obj_real["n_objects"].values,
            "k-",
            lw=1.5,
            label="実機 追跡物体数",
            alpha=0.8,
    ...
    if not df_obj_sim.empty:
        ax_obj.plot(
            df_obj_sim["t_rel"].values,
            df_obj_sim["n_objects"].values,
            color="#e05c00",
            lw=1.5,
            ls="--",
            label="シム 追跡物体数",
