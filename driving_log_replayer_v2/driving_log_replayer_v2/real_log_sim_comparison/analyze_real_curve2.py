...
for t_mark in [0, 5, 10, 15, 20, 25]:
    ...
    if row.empty:
        continue
    ax_map.scatter(row["x"], row["y"], c="red" if t_mark == 0 else "gray", s=50, zorder=5)
    ax_map.annotate(
        f"t={t_mark}s",
...
if not cmd.empty:
    ax_vel.plot(
        cmd["tr"].values, cmd["cmd_vel"].values, color="tab:blue", lw=1.5, ls="--", label="速度指令"
    )
...
if not steer.empty:
    ...
    for label, ph in [("t=0〜5s", phase1), ("t=5〜15s", phase2), ("t=15〜25s", phase3)]:
        if ph.empty:
            continue
        ph_err = (
            steer_error[phase1.index[0] - steer.index[0] : phase1.index[-1] - steer.index[0] + 1]
            if ph is phase1
            else steer_error[
                phase1.index[-1] - steer.index[0] + 1 : (
                    phase2.index[-1] - steer.index[0] + 1 if ph is phase2 else None
                )
            ]
        )
        if len(ph_err) == 0:
