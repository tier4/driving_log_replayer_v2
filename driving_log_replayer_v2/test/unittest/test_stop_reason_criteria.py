import pytest
from driving_log_replayer_v2.perception.models import StopReasonEvaluation
from driving_log_replayer_v2.perception.models import StopReasonEvaluationItem
...
def test_stop_reason_eval_basic(stop_reason_eval_item):  # noqa
    ...
    result = stop_reason_eval_item.set_frame(
        {"reason": "Intersection", "timestamp": 5.0, "dist_to_stop_pos": 0.5}
    )
    assert result["StopReason"]["Result"] == "Skip"
    # Should skip: not target reason
    result = stop_reason_eval_item.set_frame(
        {"reason": "TrafficLight", "timestamp": 12.0, "dist_to_stop_pos": 0.5}
    )
    assert result["StopReason"]["Result"] == "Skip"
    # Should pass: first valid event
    result = stop_reason_eval_item.set_frame(
        {"reason": "Intersection", "timestamp": 11.0, "dist_to_stop_pos": 0.5}
    )
    assert result["StopReason"]["Result"]["Frame"] == "Success"
    # Should skip: within tolerance interval
    result = stop_reason_eval_item.set_frame(
        {"reason": "Intersection", "timestamp": 11.2, "dist_to_stop_pos": 0.5}
    )
    assert result["StopReason"]["Result"] == "Skip"
    # Should fail: valid event but out of distance
    result = stop_reason_eval_item.set_frame(
        {"reason": "Intersection", "timestamp": 12.0, "dist_to_stop_pos": 2.0}
    )
    assert result["StopReason"]["Result"]["Frame"] == "Fail"
    # Should pass: valid event after tolerance interval
    result = stop_reason_eval_item.set_frame(
        {"reason": "Intersection", "timestamp": 12.0 + 0.6, "dist_to_stop_pos": 0.8}
    )
    assert result["StopReason"]["Result"]["Frame"] == "Success"
...
def test_stop_reason_tn_eval_basic(stop_reason_tn_eval_item):  # noqa
    ...
    # Should skip: outside time window
    result = stop_reason_tn_eval_item.set_frame(
        {"reason": "Intersection", "timestamp": 5.0, "dist_to_stop_pos": 0.5}
    )
    assert result["StopReason"]["Result"] == "Skip"
    ...
    result = stop_reason_tn_eval_item.set_frame(
        {"reason": "Intersection", "timestamp": 13.0, "dist_to_stop_pos": 0.5}
    )
    assert result["StopReason"]["Result"]["Frame"] == "Fail"
    
    # Should pass: different reason again - good for TN
    result = stop_reason_tn_eval_item.set_frame(
        {"reason": "ObstacleStop", "timestamp": 14.0, "dist_to_stop_pos": 0.8}
    )
    assert result["StopReason"]["Result"]["Frame"] == "Success"
...
def test_stop_reason_timeout(stop_reason_eval_item):  # noqa
    ...
    assert (
        "No Intersection received during evaluation window - TN success"
        in result["StopReason"]["Info"]["Message"]
    )
    ...
    stop_reason_eval_item.set_frame(
        {"reason": "Intersection", "timestamp": 11.0, "dist_to_stop_pos": 0.5}
    )
...
def test_stop_reason_tn_timeout(stop_reason_tn_eval_item):  # noqa
    ...
    stop_reason_tn_eval_item.set_frame(
        {"reason": "Intersection", "timestamp": 15.0, "dist_to_stop_pos": 0.5}
    )
    result = stop_reason_tn_eval_item.check_timeout(20.0)
